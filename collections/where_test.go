...
import (
	"fmt"
	"github.com/flysnow-org/soha/common/collections"
	"reflect"
	"testing"
	"time"
...
func TestWhere(t *testing.T) {
	...
	for i, test := range []testt{
		...
		},
		{
			...
			key: "b", op: "in", match: []float64{3.0, 4.0, 5.0},
			expect: []map[string]float64{
				{"a": 3, "b": 4},
			},
		},
		{
			...
			key: "b", op: "in", match: []time.Time{d3, d4, d5},
		...
		},
		{
			...
			key: "B", op: "not in", match: collections.Slice("c", t, "d", "e"),
			expect: []TstX{
				{A: "a", B: "b"}, {A: "e", B: "f"},
			},
		},
		...
		},
		{
			...
			key: "b", op: "in", match: collections.Slice(3, 4, 5),
