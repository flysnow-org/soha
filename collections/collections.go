...
func (ns *Namespace) In(l interface{}, v interface{}) (bool, error) {
	...
	if !vv.Type().Comparable() {
		return false, errors.New(fmt.Sprintf("value to check must be comparable: %T", v))
	}
...
func (ns *Namespace) IsSet(a interface{}, key interface{}) (bool, error) {
	...
	default:
		fmt.Printf("WARNING: calling IsSet with unsupported type %q (%T) will always return false.\n", av.Kind(), a)
	}
...
func (ns *Namespace) Uniq(seq interface{}) (interface{}, error) {
	...
	default:
		return nil, errors.New(fmt.Sprintf("type %T not supported", seq))
	}
